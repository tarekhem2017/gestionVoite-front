...
import { getAllElecteurs } from 'services/apis';
...
const Tables = (props) => {
  ...
  const [listElecteurs, setElecteurs] = useState([])
  let pageSize = 10
  let pagesCount = 1
  ...
  function getElecteures() {
    getAllElecteurs()
      .then(data => {
        pagesCount = Math.ceil(data.length / pageSize);
        setElecteurs(data.data)
      })
      .catch(err => {
        console.log({ err })
      })
          ...
          <div className="col">
            <Card className="shadow">
              <CardHeader className="border-0">
                <h3 className="mb-0">Liste électeurs</h3>
              </CardHeader>
